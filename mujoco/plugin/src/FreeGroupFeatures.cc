...
static mujocolib::Link *FindModelRootLink(mujocolib::Model *_model)
{
  if (nullptr == _model)
    return nullptr;
  ...
  if (_model->GetLinkCount() > 0)
  {
    ...
    return static_cast<mujocolib::Link *>(&_model->GetCanonicalLink());
  }
  else
  {
    ...
    for (size_t i = 0; i < _model->GetChildCount(); ++i)
    {
      auto *rootLink = FindModelRootLink(
          static_cast<mujocolib::Model *>(&_model->GetChildByIndex(i)));
      if (nullptr != rootLink)
        return rootLink;
    }
  }
  ...
  const PoseType &_pose)
{
  ...
  // get top level model
  mujocolib::Entity *parent = link->GetParent();
  mujocolib::Entity *model = nullptr;
  while (parent && dynamic_cast<mujocolib::Model *>(parent))
  {
    model = parent;
