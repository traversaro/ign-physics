  ...
  const ForwardStep::Input & _u)
{
  ...
  std::shared_ptr<mujocolib::World> world = it->second->world;
  auto *dtDur =
...
SimulationFeatures::GetContactsFromLastStep(const Identity &_worldID) const
{
  ...
  for (const auto &c : contacts)
  {
    ...
    // but mujoco computes collisions between models
...
mujocolib::Entity &SimulationFeatures::GetModelCollision(std::size_t _id) const
{
  auto m = this->models.at(_id);
  if (!m || !m->model)
    return mujocolib::Entity::kNullEntity;

  mujocolib::Entity &link = m->model->GetCanonicalLink();
