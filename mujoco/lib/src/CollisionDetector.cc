...
class ignition::physics::mujocolib::CollisionDetectorPrivate
{
...
std::vector<Contact> CollisionDetector::CheckCollisions(
    ...
    bool _singleContact)
{
  IGN_PROFILE("mujocolib::CollisionDetector::CheckCollisions");
  ...
  for (auto it = _entities.begin(); it != _entities.end(); ++it)
  {
    ...
    for (const auto &nId : result)
    {
      ...
      {
        Contact c;
        // mujoco checks collisions in the model level so contacts are associated
