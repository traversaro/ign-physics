...
class ignition::physics::mujocolib::ModelPrivate
{
  ...
  /// \brief Joints in the model
  public: std::vector<std::size_t> jointIds;
...
Entity &Model::AddLink()
{
  ...
  links.push_back((Link*)it->second.get());
...
//////////////////////////////////////////////////
Link* Model::GetLinkFromName(std::string name)
{
  for (auto linkPtr: this->links) 
  {
    if (linkPtr->GetName() == name) {
      return linkPtr;
    }
  }
  return nullptr;
...
//////////////////////////////////////////////////
Entity &Model::AddJoint()
{
  std::size_t jointId = Entity::GetNextId();

  const auto[it, success]  = this->GetChildren().insert(
      {jointId, std::make_shared<Joint>(jointId)});
  this->dataPtr->jointIds.push_back(jointId);

  it->second->SetParent(this);
  this->ChildrenChanged();
  joints.push_back((Joint*)it->second.get());
  return *it->second.get();
}

//////////////////////////////////////////////////
std::size_t Model::GetJointCount() const
{
  return this->dataPtr->jointIds.size();
}

//////////////////////////////////////////////////
Joint* Model::GetJointFromName(std::string name)
{
  for (auto jointPtr: this->joints) 
  {
    if (jointPtr->GetName() == name) {
      return jointPtr;
    }
  }
}
...
{
  IGN_PROFILE("mujocolib::Model::GetLinearVelocity");
...
{
  IGN_PROFILE("mujocolib::Model::GetAngularVelocity");
...
{
  IGN_PROFILE("mujocolib::Model::UpdatePose");
