...
#ifndef IGNITION_PHYSICS_MUJOCO_LIB_SRC_MODEL_HH_
#define IGNITION_PHYSICS_MUJOCO_LIB_SRC_MODEL_HH_
...
/// \brief Model class
class IGNITION_PHYSICS_MUJOCOLIB_VISIBLE Model : public Entity
{
  /// \brief Constructor
  public: Model();
  ...
  public: Link* GetLinkFromName(std::string name);

    /// \brief Add a joint
  /// \return Newly created joint
  public: Entity &AddJoint();

  /// \brief Get the number of joint in the model
  /// \return Number of joints in the model
  public: std::size_t GetJointCount() const;

  public: Joint* GetJointFromName(std::string name);
